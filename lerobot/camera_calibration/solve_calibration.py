...
def estimate_tag_pose(finger_pose, ft_sensor=True):
    """
    ...
    ### this offset assumes using the default TCP from xarm
    ### TODO: change this for the offset from the tag to the aloha eef center
    # t_tag_to_finger = np.array([2.0, 0, -11.088]) / 100.    
    t_tag_to_finger = np.array([0, 0, 0]) / 100.    
    tag_to_finger = np.eye(4)
    tag_to_finger[:3, 3] = t_tag_to_finger
    tag_pose = np.dot(finger_pose, tag_to_finger)

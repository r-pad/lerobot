...
class DP3Config(PreTrainedConfig):
    """Configuration class for DP3 (3D Diffusion Policy).
    ...
    pointnet_type: str = "pointnet"  # "pointnet" or "pointnet_xyz"
    pointcloud_encoder_cfg: dict = field(default_factory=lambda: {
        "out_channels": 1024,
        "use_layernorm": False,
        "final_norm": "none",
        "use_projection": True,
    ...
    # DP3 specific conditioning
    condition_type: str = "film"  # "film", "cross_attention_film", "cross_attention_add", etc.
    ...
    agent_pos_key: str = "observation.state"  # or "observation.agent_pos"
    imagination_key: str = "observation.imagin_robot"  # optional
    ...
    def __post_init__(self):
        ...
        # DP3 specific validations
        if self.pointnet_type not in ["pointnet", "pointnet_xyz"]:
            raise ValueError(f"Invalid pointnet_type: {self.pointnet_type}")

        if "cross_attention" in self.condition_type and not self.obs_as_global_cond:
            raise ValueError("Cross-attention conditioning requires obs_as_global_cond=True")

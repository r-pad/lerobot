...
class FrankaOSCController():
    def __init__(self,
                 ...
                 visualizer=False,
                 verbose=False,
                 pos_action_gain=20.0,
                 rot_action_gain=2.0,
                 action_smoothing=0.0,
                 action_pos_limit=1.0,
                 action_rot_limit=0.5):
        ...
        self.action_pos_limit = action_pos_limit
        self.action_rot_limit = action_rot_limit
    ...
    def _osc_move(
            ...
            action_pos_limit=None,
            action_rot_limit=None,
            ):
        ...
        action_smoothing = self.action_smoothing if action_smoothing is None else action_smoothing
        action_pos_limit = self.action_pos_limit if action_pos_limit is None else action_pos_limit
        action_rot_limit = self.action_rot_limit if action_rot_limit is None else action_rot_limit
        action_smoothing = float(np.clip(action_smoothing, 0.0, 0.98))

        prev_action_pos = np.zeros(3)
        prev_action_axis_angle = np.zeros(3)
        ...
        while step_count < num_steps:
            ...
            action_pos = (target_pos - current_pos).flatten() * self.pos_action_gain
            action_axis_angle = axis_angle_diff.flatten() * self.rot_action_gain
            action_pos = np.clip(action_pos, -action_pos_limit, action_pos_limit)
            # logger.info(f"Action pos {action_pos.tolist()}. Current pos {current_pos.flatten().tolist()}. Target pos {target_pos.flatten().tolist()}")
            action_axis_angle = np.clip(action_axis_angle, -action_rot_limit, action_rot_limit)
            if action_smoothing > 0.0:
                action_pos = action_smoothing * prev_action_pos + (1.0 - action_smoothing) * action_pos
                action_axis_angle = (
                    action_smoothing * prev_action_axis_angle
                    + (1.0 - action_smoothing) * action_axis_angle
                )
                prev_action_pos = action_pos
                prev_action_axis_angle = action_axis_angle
            ...
            if self.verbose:
                print(self.controller_type, self.controller_cfg)
                print(action)
    ...
    def target_to_flange_pose(self, target_pos, target_quat):
        """Convert a user-facing target tip pose into the raw flange pose commanded by OSC."""
        target_pos = np.array(target_pos).reshape(3, 1)
        target_quat = np.array(target_quat)
        return self._apply_offset((target_pos, target_quat))

...
Based on the LEAP Hand dynamixel client.
"""
...
class DynamixelClient:
    ...
    def read_pos_vel_cur(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Returns the current positions, velocities, and currents."""
...
class DynamixelPosVelCurReader(DynamixelReader):
    ...
    def __init__(self, client, motor_ids, pos_scale=1.0, vel_scale=1.0, cur_scale=1.0):
        super().__init__(client, motor_ids, address=ADDR_PRESENT_POS_VEL_CUR, size=LEN_PRESENT_POS_VEL_CUR)
    ...
    def _update_data(self, index, motor_id):
        cur = unsigned_to_signed(self.operation.getData(motor_id, ADDR_PRESENT_CURRENT, LEN_PRESENT_CURRENT), size=2)
        vel = unsigned_to_signed(self.operation.getData(motor_id, ADDR_PRESENT_VELOCITY, LEN_PRESENT_VELOCITY), size=4)
        pos = unsigned_to_signed(self.operation.getData(motor_id, ADDR_PRESENT_POSITION, LEN_PRESENT_POSITION), size=4)
...
class DynamixelPosVelReader(DynamixelReader):
    ...
    def __init__(self, client, motor_ids, pos_scale=1.0, vel_scale=1.0):
        super().__init__(client, motor_ids, address=ADDR_PRESENT_POS_VEL, size=LEN_PRESENT_POS_VEL)
        self.pos_scale = pos_scale
        self.vel_scale = vel_scale
...
class DynamixelPosReader(DynamixelReader):
    ...
    def __init__(self, client, motor_ids, pos_scale=1.0, vel_scale=1.0, cur_scale=1.0):
        super().__init__(client, motor_ids, address=ADDR_PRESENT_POSITION, size=LEN_PRESENT_POSITION)
        self.pos_scale = pos_scale
...
class DynamixelVelReader(DynamixelReader):
    ...
    def __init__(self, client, motor_ids, pos_scale=1.0, vel_scale=1.0, cur_scale=1.0):
        super().__init__(client, motor_ids, address=ADDR_PRESENT_VELOCITY, size=LEN_PRESENT_VELOCITY)
        self.vel_scale = vel_scale
...
class DynamixelCurReader(DynamixelReader):
    ...
    def __init__(self, client, motor_ids, pos_scale=1.0, vel_scale=1.0, cur_scale=1.0):
        super().__init__(client, motor_ids, address=ADDR_PRESENT_CURRENT, size=LEN_PRESENT_CURRENT)
        self.cur_scale = cur_scale

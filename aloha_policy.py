...
try:
    ...
    DataTransformFn = transforms.DataTransformFn
except ImportError:
...
def normalize_state_mean_std(state: torch.Tensor | np.ndarray, mean: torch.Tensor | np.ndarray, std: torch.Tensor | np.ndarray) -> torch.Tensor | np.ndarray:
    """Normalize state using mean and std."""
    # Ensure std has no zeros to avoid division by zero
    ...
    # Clamp to [-1, 1] if tensor (OpenPI expectation)
...
def unnormalize_action_mean_std(action: torch.Tensor | np.ndarray, mean: torch.Tensor | np.ndarray, std: torch.Tensor | np.ndarray) -> torch.Tensor | np.ndarray:
    """Unnormalize action using mean and std."""
